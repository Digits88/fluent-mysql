...
var QueryBuilder = function(conn) {
    ...
    /*
     Constructs a group by statement from an array of field names.
     */
    this.groupByFromFields = function (groupFields) {
        if (!groupFields || groupFields.length == 0) return "";

        return " GROUP BY " + groupFields.reduce(function (prev, curr) {
    ...
    this.__defineGetter__('groupBySegment', function() {
        return this.groupByFromFields(this.groupFields);
    })
};
